...
├── docs/
│   └── comparison-matrix.md           # Framework comparison (ADF vs Temporal vs Step Functions)
├── azure-durable-functions/           # ✅ Azure Durable Functions implementation
│   ├── README.md                      # Setup & architecture
│   ├── SPEC.md                        # Technical specifications
│   ├── function_app.py                # Orchestrator & HTTP triggers
│   ├── storage_helper.py              # Blob storage utilities
│   ├── activities/                    # Activity blueprints
│   │   ├── upload_pdf.py
│   │   ├── split_pdf.py
│   │   ├── extract_invoice.py
│   │   └── aggregate_invoice.py
│   ├── api_bruno/                     # API test collection
│   ├── data/                          # Sample PDFs
│   ├── tools/                         # Test & load test scripts
│   ├── pyproject.toml                 # Dependencies
│   ├── host.json                      # Runtime config
│   └── local.settings.json            # Dev settings
├── temporal/                          # 🔄 [Planned] Temporal implementation
└── aws-step-functions/                # 🔄 [Planned] Step Functions implementation
```
...
- **Orchestrator**: `function_app.py` → `invoice_orchestrator` (must be deterministic!)
...
Always include the current date and time in the response footer using short format (e.g., `12/15/2025 2:30 PM`).
